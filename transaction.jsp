	...
	<table class="container text-dark  table-striped   text-center  table-bordered" >
        ...
        <%
			try{
				...
				PreparedStatement check_stm=con.prepareStatement("select distinct  * from recipt  order by TRANSACTION_ID desc  ");
				ResultSet rs=check_stm.executeQuery(); 
				while (rs.next()) {
                    String name = rs.getString("EMAIL");
					String email = rs.getString("PRODUCT");
					String pass = rs.getString("NO_OF_PRODUCT");
					String userid = rs.getString("COST");
					String userid1 = rs.getString("ADDRESS");
					String userid2= rs.getString("TRANSACTION_ID");
		%>
					<tr class=" text-dark  w-100" style="height: 45px;">
						...
						<td><%= userid1%></td>
						<td><%= userid %></td>
                        <!-- <td ><form action="admindelete.jsp" method="POST"> <button class="btn btn-info w-50"  name="del" value="<%= userid %>">Delete</button></form></td> -->
					</tr>
		<%
				}
				con.close();
